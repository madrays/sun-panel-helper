...
import { generateCSS } from './service'
import type { GlobalFontParams } from './types'

// 运行时已在backend目录下，使用join拼接路径
const outputPath = join('custom', 'index.css')

// 组件标记
const startMark = '/* Sun-Panel-Helper CSS Start: global-font */'
const endMark = '/* Sun-Panel-Helper CSS End: global-font */'
...
export async function deploy(css: string): Promise<void> {
  try {
    ...
    const currentIndex = order.indexOf('global-font');
