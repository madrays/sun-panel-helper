...
// 组件标记
const startMark = '/* Sun-Panel-Helper JS Start: music-player */';
const endMark = '/* Sun-Panel-Helper JS End: music-player */';
...
export async function deploy(js: string): Promise<void> {
  try {
    ...
    const currentIndex = order.indexOf('music-player');
...
(function() {
    try {
        ...
        console.error('[Sun-Panel-Helper] Error in music-player:', e);
    }
})();`;
